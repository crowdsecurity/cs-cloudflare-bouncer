...
def test_install_no_crowdsec(project_repo, bouncer_binary, must_be_root):
    ...
    c.expect(f"Please configure '{CONFIG}'")
    c.expect(f"Configuration can be autogenerated using /usr/local/bin/{BOUNCER} -g <CF_TOKEN_1>,<CF_TOKEN_2>")
    c.expect(f"After configuration run the command 'systemctl start {BOUNCER}.service' to start the bouncer")
    c.wait()
    assert c.terminated
